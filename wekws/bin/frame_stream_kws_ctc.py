...
def get_args():
    parser = argparse.ArgumentParser(description='detect keywords online (frame streaming).')
...
class FrameStreamKeyWordSpotter(torch.nn.Module):
    """
    帧流式关键词检测器
    每次处理一帧特征，实现真正的帧级流式处理
    """
    ...
    def __init__(
        ...
        self.wave_buffer = np.array([])  # 音频缓冲区，用于累积足够提取一帧的音频
        ...
        # 计算一帧需要的音频样本数
        self.frame_shift_samples = int(self.frame_shift / 1000 * self.sample_rate)
        self.frame_length_samples = int(self.frame_length / 1000 * self.sample_rate)
        ...
        if self.context_expansion:
            ...
            self.right_context = dataset_conf['context_expansion_conf'][
                'right']
        self.feature_buffer = []  # 特征缓冲区，用于上下文扩展
    ...
    def accept_wave_frame(self, wave_bytes):
        """
        接受音频字节流，提取一帧特征
        帧流式处理：每次只处理一帧所需的音频量
        
        :param wave_bytes: bytes格式的PCM音频数据
        :return: 单帧特征向量 (feature_dim,)，如果音频不够则返回None
        """
        assert isinstance(wave_bytes, bytes), \
            "please make sure the input format is bytes(raw PCM)"
        
        # 转换bytes为numpy数组
        data = []
        for i in range(0, len(wave_bytes), 2):
            value = struct.unpack('<h', wave_bytes[i:i + 2])[0]
            data.append(value)
        
        wave = np.array(data)
        self.wave_buffer = np.append(self.wave_buffer, wave)
        
        # 检查是否有足够的音频来提取一帧特征
        # 需要至少frame_length_samples的音频来提取第一帧
        if len(self.wave_buffer) < self.frame_length_samples:
            return None
        
        # 提取特征：使用buffer中的音频，fbank会自动计算能提取多少帧
        wave_tensor = torch.from_numpy(self.wave_buffer).float()
        wave_tensor = wave_tensor.unsqueeze(0)  # (1, samples)
        ...
        # 计算应该取哪一帧（最后一帧，即新的一帧）
        # 基于buffer长度计算：buffer中有 frame_length + n*frame_shift 的音频
        # 可以提取 n+1 帧，我们取最后一帧（索引为n）
        num_frames = len(feats)
        frame_idx = num_frames - 1  # 取最后一帧（新的一帧）
        
        feat_frame = feats[frame_idx]  # (feature_dim,)
        
        # 更新缓冲区：移除已处理的音频（移动frame_shift_samples）
        # 但保留足够的音频用于下一帧的提取
        self.wave_buffer = self.wave_buffer[self.frame_shift_samples:]
        
        # 处理上下文扩展
        if self.context_expansion:
            self.feature_buffer.append(feat_frame)
            # 保持足够的上下文
            if len(self.feature_buffer) > self.left_context + self.right_context + 1:
                self.feature_buffer.pop(0)
            
            # 如果缓冲区有足够的帧，构建上下文特征
            if len(self.feature_buffer) >= self.left_context + self.right_context + 1:
                # 构建上下文窗口：以最后一帧为中心
                center_idx = len(self.feature_buffer) - 1
                start_idx = max(0, center_idx - self.left_context)
                end_idx = min(len(self.feature_buffer), center_idx + self.right_context + 1)
                ctx_feat = torch.cat(self.feature_buffer[start_idx:end_idx])
                
                # 如果上下文不够，用边界帧填充
                if start_idx > 0:
                    # 左侧不够，用第一帧填充
                    pad_left = [self.feature_buffer[0]] * (self.left_context - (center_idx - start_idx))
                    ctx_feat = torch.cat(pad_left + [ctx_feat])
                if end_idx < len(self.feature_buffer):
                    # 右侧不够，用最后一帧填充
                    pad_right = [self.feature_buffer[-1]] * (self.right_context - (end_idx - center_idx - 1))
                    ctx_feat = torch.cat([ctx_feat] + pad_right)
                
                return ctx_feat.to(self.device)
            else:
                # 如果上下文不够，用replicate padding
                if len(self.feature_buffer) == 1:
                    # 第一帧，复制填充左侧和右侧
                    padded = [feat_frame] * (self.left_context + 1 + self.right_context)
                    ctx_feat = torch.cat(padded)
                    return ctx_feat.to(self.device)
                else:
                    # 部分上下文，用边界帧填充
                    first_frame = self.feature_buffer[0]
                    last_frame = self.feature_buffer[-1]
                    # 左侧用第一帧填充，右侧用最后一帧填充
                    pad_left = [first_frame] * (self.left_context - len(self.feature_buffer) + 1)
                    pad_right = [last_frame] * self.right_context
                    ctx_feat = torch.cat(pad_left + self.feature_buffer + pad_right)
                    return ctx_feat.to(self.device)
        else:
            return feat_frame.to(self.device)

    def forward_frame(self, feat_frame):
        """
        处理一帧特征，进行解码和检测
        
        :param feat_frame: 单帧特征向量 (feature_dim,) 或带上下文的特征
        :return: 检测结果字典
        """
        if feat_frame is None:
            return {}
        
        # 添加batch和sequence维度: (1, 1, feature_dim)
        feat_frame = feat_frame.unsqueeze(0).unsqueeze(0)
        
        # 模型前向传播
        logits, self.in_cache = self.model(feat_frame, self.in_cache)
        probs = logits.softmax(2)  # (batch_size, seq_len, vocab_size)
        prob = probs[0, 0].cpu()  # 取第一帧的概率分布 (vocab_size,)
        ...
        # 更新帧计数（在downsampling之后）
        self.total_frames += self.downsampling
        
        # 检查是否需要重置（超过max_frames）
    ...
    def forward(self, wave_bytes):
        """
        便捷方法：接受音频字节流，自动提取特征并处理一帧
        
        :param wave_bytes: bytes格式的PCM音频数据
        :return: 检测结果字典
        """
        feat_frame = self.accept_wave_frame(wave_bytes)
        return self.forward_frame(feat_frame)
...
def demo():
    ...
    kws = FrameStreamKeyWordSpotter(args.checkpoint, args.config, args.token_file,
    ...
    if args.wav_path:
        ...
        # 帧流式处理：每次输入一帧所需的音频量
        # frame_shift_samples = int(0.01 * 16000) = 160 samples (假设frame_shift=10ms)
        # 每次输入160*2=320字节（16bit = 2字节）
        frame_shift_bytes = int(kws.frame_shift / 1000 * kws.sample_rate) * 2
        
        for i in range(0, len(wav), frame_shift_bytes):
            chunk_wav = wav[i:min(i + frame_shift_bytes, len(wav))]
            if len(chunk_wav) < frame_shift_bytes:
                # 最后不足一帧的数据，填充或跳过
                break
